...
def all_users(ack, say, command):
    ...
    users = get_all_users()
    for user in users:
        say(f"<@{user.id}> в стриме {user.group} в команде {user.command} активный {user.is_active}")
...
def users():
    ...
    add_group("U01KW38GFHQ", "promofabric")
    add_group("U01AVKUKECC", "credits")
    add_group("U01DRQFPB8X", "promofabric")
    return "OK"
